...
together), drop any file named `*icon.*` (e.g. `favicon.ico`,
`icon.png`, `tab-icon.svg`) in the same folder as the generated
`index.html` - it's picked up automatically on the next `npmpi gen`, no
config needed, and used for both. If no icon file is found, the title
and subtitle just show centered on their own, exactly as before (e.g.
